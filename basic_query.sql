...
with kill_data as
	(select file,att_team,att_side,att_id,COUNT(*) as kills
	from(select distinct * 
		from(select file, round, tick,att_team,vic_team,att_side,vic_side,hp_dmg,arm_dmg,att_id,vic_id 
			from esea_dmg
			) as a1 natural join (select file,round,tick,att_team,vic_team,att_side,vic_side from esea_kills) as a2
...
,
death_data as
	(select file,  vic_team, vic_side, vic_id, COUNT(*) as deaths
	from(
		select distinct *
		from(select file, round, tick,att_team,vic_team,att_side,vic_side,hp_dmg,arm_dmg,att_id,vic_id 
			from esea_dmg
...
from kill_data natural join death_data
where att_team=vic_team and att_side =vic_side and att_id = vic_id
;
--Highest Kill/Death ratio per match--
with kill_data as
	(select file,att_team,att_side,att_id,COUNT(*) as kills
	from(select distinct * 
		from(select file, round, tick,att_team,vic_team,att_side,vic_side,hp_dmg,arm_dmg,att_id,vic_id 
...
where rank <=1
;
--Lowest Kill/Death ratio per match--
with kill_data as
	(select file,att_team,att_side,att_id,COUNT(*) as kills
	from(select distinct * 
		from(select file, round, tick,att_team,vic_team,att_side,vic_side,hp_dmg,arm_dmg,att_id,vic_id 
...
with assists_data as
	(select file,att_team,att_side,att_id,COUNT(*) as assists
	from(select file, round, tick,att_team,vic_team,att_side,vic_side,hp_dmg,arm_dmg,att_id,vic_id 
		from esea_dmg
		except
		select *
...
,
value_data as
	(select file,att_team,att_side,att_id,(kills+assists) as value from kill_data natural join assists_data)
