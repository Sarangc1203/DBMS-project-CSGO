...
with kill_data2 as (select att_team,att_side,att_id,SUM(count) as kills from kill_data group by att_team,att_side,att_id),
assist_data2 as (select att_team,att_side,att_id,SUM(count) as assists from assist_data group by att_team,att_side,att_id),
death_data2 as (select vic_team as att_team,vic_side as att_side,vic_id as att_id,SUM(count) as deaths from death_data group by vic_team,vic_side,vic_id),
mvp2 as (select att_team,att_side,att_id,COUNT(*) as num_mvp from mvp group by att_team,att_side,att_id) 

select att_team,att_side,att_id,kills,assists,deaths,ROUND( cast(kills as decimal)/deaths,2) as ratio,num_mvp from kill_data2 natural join assist_data2 natural join death_data2 natural join mvp2
;
